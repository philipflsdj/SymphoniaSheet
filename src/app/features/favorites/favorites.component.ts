...
import { SheetCardComponent } from '../sheets/components/sheet-card.component';
  ...
  template: `
    <div class="flex flex-col gap-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-extrabold text-surface-900">Meus Favoritos</h1>
          <p class="text-surface-500 mt-1">Acesse rapidamente as partituras que você marcou como favoritas.</p>
        </div>
      ...
      @if (isLoading()) {
        ...
        <app-empty-state 
          ...
          description="Você ainda não adicionou nenhuma partitura aos favoritos ou os filtros não retornaram resultados."
        >
          <app-button variant="outline" (onClick)="clearFilters()">Limpar Filtros</app-button>
          <app-button (onClick)="router.navigate(['/sheets'])" class="ml-2">Explorar Partituras</app-button>
        </app-empty-state>
      } @else {
...
export class FavoritesComponent implements OnInit {
  ...
  filter = signal<SheetFilter>({
    ...
    isFavorite: true // Only show favorites
  });
  ...
  clearFilters() {
    this.filter.set({
      ...
      difficulty: undefined,
      materialType: undefined,
      isFavorite: true // Keep isFavorite true
    });
    this.loadSheets();
  }
  ...
  onToggleFavorite(sheet: SheetMusic) {
    ...
    // Reload to remove from the list if it's no longer a favorite
    if (!sheet.isFavorite) {
      this.loadSheets();
    }
  }
